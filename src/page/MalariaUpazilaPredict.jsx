...
export default function MalariaRiskTracker() {
  ...
  useEffect(() => {
    fetch("/upazila_simplified_final.json")
      .then((res) => res.json())
      .then((data) => {
        setGeoJson(data);
        ...
        setSelectedDistricts([
          ...new Set(h.divisions.flatMap((div) => h.districts[div] || [])),
        ]);
        setSelectedUpazilas([
          ...new Set(
            Object.values(h.districts).flatMap((dists) =>
              dists.flatMap((dist) => h.upazilas[dist] || [])
            )
          ),
        ]);
      });
  ...
  useEffect(() => {
    ...
    fetch("/lmis-data.json")
      ...
      .then((apiData) => {
        ...
        toast?.success(
          `Successfully loaded ${filtered.length} malaria records from local data`
        );
      })
      .catch((error) => {
        ...
        setActualData([]);
        toast?.error(
          "Failed to load local malaria data. Please check if data file exists."
        );
      });
  }, [geoJson]);
  ...
  const hierarchy = useMemo(
    () =>
      geoJson
        ? buildHierarchy(geoJson.features)
  ...
  useEffect(() => {
    ...
    const validDists = [
      ...new Set(
        selectedDivisions.flatMap((div) => hierarchy.districts[div] || [])
      ),
    ];
  ...
  useEffect(() => {
    ...
    const validUpa = [
      ...new Set(
        selectedDistricts.flatMap((dist) => hierarchy.upazilas[dist] || [])
      ),
    ];
    setSelectedUpazilas(validUpa);
    ...
    return months.map((month) => {
      ...
      upazilas.forEach((upz) => {
        ...
        const pred = forecastResults.find(
          (d) => d.upa_name === upz && d.forecast_month === month.code
        );
    ...
    return geojson.features
      .map((feature) => {
        ...
        if (type === "forecast") {
          ...
          cases =
            pred && pred.pred_cases !== undefined ? Number(pred.pred_cases) : 0;
        } else if (type === "actual") {
          const actual = getActual(upaID);
          cases =
            actual && actual.CASEE !== undefined ? Number(actual.CASEE) : 0;
        }
          ...
          if (pred && pred.pred_cases !== undefined) {
            html += `
              ...
              <tr>
                  ...
                  <td style="padding: 2px 4px;">${pred.pred_cases.toFixed(
                    0
                  )}</td>
              </tr>`;
          } else {
...
function buildHierarchy(features) {
  ...
  return {
    ...
    districts: Object.fromEntries(
      Object.entries(dists).map(([k, v]) => [k, [...v]])
    ),
    upazilas: Object.fromEntries(
      Object.entries(upas).map(([k, v]) => [k, [...v]])
    ),
  };
}
  ...
  return [
    {
      ...
      code: prev.toISOString().slice(0, 10),
    },
    {
      label: `${monthNames[currentMonth]} ${currentYear}`,
      code: curr.toISOString().slice(0, 10),
    },
    {
      label: `${monthNames[next.getMonth()]} ${next.getFullYear()}`,
      code: next.toISOString().slice(0, 10),
    },
  ];
}
          ...
          >
              ...
              >
                <span
                  style={{
                    ...
                    animation: "bip-pulse 1s infinite alternate",
                  }}
                />{" "}
                threshold
              </div>
            ...
            {geojson && (
              <GeoJSONLayer
                geojson={geojson}
                forecastResults={
                  type === "forecast" ? forecastResults : undefined
                }
