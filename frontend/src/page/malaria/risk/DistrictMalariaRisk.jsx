...
export default function MalariaRiskDistrict() {
    ...
    // Disease map controls
    const [showDiseaseMap, setShowDiseaseMap] = useState(true);
    const [diseaseResolution, setDiseaseResolution] = useState("District"); // "Upazila" | "District"
    ...
    function dissolveDistrictPolygons(upazilas, districtProperty) {
        const grouped = {};
        upazilas.features.forEach(f => {
            const district = f.properties[districtProperty];
            if (!grouped[district]) grouped[district] = [];
            grouped[district].push(f);
        });

        // For each district: merge all polygons
        return Object.keys(grouped).map(district => {
            let unioned = grouped[district][0];
            for (let i = 1; i < grouped[district].length; i++) {
                unioned = turf.union(unioned, grouped[district][i]) || unioned;
            }
            // output unioned feature
            return unioned;
        });
    }
    ...
    useEffect(() => {
        // Use malaria risk geojson as source
        fetch("/malaria-risk-district.geojson")
            .then(res => res.json())
            .then(data => {
                setGeoJson(data);
    ...
    const selectedUpaObjs = selectedUpazilas.map(name => {
        // Match by upazila name (supports both UPA_NAME and upazila keys)
        const feature = geoJson.features.find(f => {
            const upa = (f.properties.UPA_NAME || f.properties.upazila || "").trim().toLowerCase();
            return upa === name.trim().toLowerCase();
        });
        return feature
    ...
    const filteredGeoJson = useMemo(() => {
        ...
        const features = geoJson.features.filter(f => {
            const divName = f.properties.DIV_NAME || f.properties.division || "";
            const distName = f.properties.DIS_NAME || f.properties.district || "";
            const upaName = f.properties.UPA_NAME || f.properties.upazila || "";
            return (
                selectedDivisions.includes(divName) &&
                selectedDistricts.includes(distName) &&
                selectedUpazilas.includes(upaName)
            );
        });
    ...
    // Grouped GeoJSON by District (built from currently filtered features)
    const groupedDistrictGeoJson = useMemo(() => {
        if (!filteredGeoJson) return null;
        return buildDistrictDissolvedGeoJson(filteredGeoJson);
    }, [filteredGeoJson]);
                    ...
                    {showDiseaseMap && (
                        <div className="mt-2 flex items-center gap-4 text-sm">
                            <label className="flex items-center gap-1 cursor-pointer">
                                <input
                                    type="radio"
                                    name="disease-resolution"
                                    value="Upazila"
                                    checked={diseaseResolution === "Upazila"}
                                    onChange={() => setDiseaseResolution("Upazila")}
                                />
                                Upazila
                            </label>
                            <label className="flex items-center gap-1 cursor-pointer">
                                <input
                                    ...
                                    name="disease-resolution"
                                    value="District"
                                    checked={diseaseResolution === "District"}
                                    onChange={() => setDiseaseResolution("District")}
                                />
                                District
                            </label>
                        </div>
    ...
    useEffect(() => {
        ...
        const layer = L.geoJSON(geojson, {
            ...
            onEachFeature: (feature, lyr) => {
                const upaID = feature?.properties?.UpazilaID;
                const upaName = feature?.properties?.UPA_NAME || feature?.properties?.upazila;
...
function buildHierarchy(features) {
    ...
    features.forEach(f => {
        const div = f.properties.DIV_NAME || f.properties.division;
        const dist = f.properties.DIS_NAME || f.properties.district;
        const upa = f.properties.UPA_NAME || f.properties.upazila;
...
function buildDistrictDissolvedGeoJson(geojson) {
    if (!geojson || !geojson.features) return null;

    // Build sums and labels by DistrictID
    const metaById = new Map();
    for (const f of geojson.features) {
        if (!f?.geometry) continue;
        const p = f.properties || {};
        const id = p.DistrictID ?? p.DIS_NAME ?? p.district;
        if (id == null) continue;
        ...
        if (!metaById.has(key)) {
            metaById.set(key, {
                id: p.DistrictID ?? id,
                name: p.DIS_NAME || p.district || "",
                division: p.DIV_NAME || p.division || "",
                sums: {},
            });
        }
        const m = metaById.get(key);
        for (const k of Object.keys(p)) {
            if (/^\d{4}$/.test(k)) {
                const v = Number(p[k]);
                if (Number.isFinite(v)) m.sums[k] = (m.sums[k] || 0) + v;
            }
        }
    }

    // Prepare cleaned features with DistrictID for dissolve
    const cleaned = geojson.features
        .filter(f => f?.geometry)
        .map(f => {
            const p = f.properties || {};
            const id = p.DistrictID ?? p.DIS_NAME ?? p.district;
            const base = { type: "Feature", properties: { DistrictID: id }, geometry: f.geometry };
            try {
                const cc = turf.cleanCoords(base);
                if (cc?.geometry) return cc;
            } catch { }
            return base;
        });
    ...
    // First try: turf.dissolve by DistrictID
    try {
        let dissolved = turf.dissolve(fc, { propertyName: "DistrictID" });
        // If dissolve returns empty, try repair via buffer(0) and dissolve again
        if (!dissolved?.features?.length) {
            const buffered = {
                type: "FeatureCollection",
                features: cleaned.map(f => {
                    try { return turf.buffer(f, 0); } catch { return f; }
                })
            };
            dissolved = turf.dissolve(buffered, { propertyName: "DistrictID" });
        }

        if (dissolved?.features?.length) {
            const features = dissolved.features.map(df => {
                const key = String(df.properties?.DistrictID);
                const m = metaById.get(key) || { id: df.properties?.DistrictID, name: "", division: "", sums: {} };
                return {
                    type: "Feature",
                    properties: {
                        DistrictID: m.id,
        ...
        // fall through to iterative-safe union fallback
    }

    // Fallback: iterative union per group (with safe concat when union fails)
    const groups = new Map();
    const getMultiCoords = (f) => (f.geometry?.type === "Polygon" ? [f.geometry.coordinates] : f.geometry?.coordinates || []);
    const concatMulti = (a, b) => ({ type: "Feature", properties: {}, geometry: { type: "MultiPolygon", coordinates: [...getMultiCoords(a), ...getMultiCoords(b)] } });
    const safeUnion = (a, b) => {
        try {
            const res = turf.union(a, b);
            if (res) return res;
        } catch { }
        return concatMulti(a, b);
    };

    for (const f of cleaned) {
        const key = String(f.properties.DistrictID);
        if (!groups.has(key)) groups.set(key, []);
        groups.get(key).push(f);
    }

    const features = Array.from(groups.entries()).map(([key, feats]) => {
        let merged = feats[0];
        for (let i = 1; i < feats.length; i++) merged = safeUnion(merged, feats[i]);
        const m = metaById.get(String(key)) || { id: key, name: "", division: "", sums: {} };
        return {
            type: "Feature",
            properties: {
                DistrictID: m.id,
...
// Build grouped MultiPolygon features by DistrictID and aggregate yearly values
function groupByDistrict(geojson) {
    const groups = new Map();
    ...
    for (const f of geojson.features || []) {
        ...
        // Track numeric year keys (e.g., "2012".."2025")
        Object.keys(props).forEach(k => {
            if (/^\d{4}$/.test(k)) yearKeys.add(k);
        });
        ...
        if (!groups.has(key)) {
            groups.set(key, {
                id: districtId,
                name: props.DIS_NAME || props.district || "",
                division: props.DIV_NAME || props.division || "",
                coords: [],
                sums: {},
            });
        }
        ...
        // Aggregate geometry to MultiPolygon (collect each polygon ring set)
        if (f.geometry?.type === "Polygon") {
            g.coords.push(f.geometry.coordinates);
        } else if (f.geometry?.type === "MultiPolygon") {
            for (const poly of f.geometry.coordinates) g.coords.push(poly);
        }
...
function DiseaseGeoJSONLayer({ geojson, year, isDistrict = false }) {
    const map = useMap();
    ...
    useEffect(() => {
        ...
        const values = (geojson.features || [])
            .map(f => Number(f.properties?.[yearKey]))
        ...
        const getColor = (v) => {
            ...
            const stops = ["#4575b4", "#91bfdb", "#ffffbf", "#fdae61", "#d73027"];
            const idx = Math.max(0, Math.min(stops.length - 1, Math.floor(t * stops.length)));
            return stops[idx];
        };

        const layer = L.geoJSON(geojson, {
            style: feature => {
                const val = Number(feature?.properties?.[yearKey]);
                return {
                    color: "#222",
                    weight: isDistrict ? 0.8 : 0.2,
            ...
            onEachFeature: (feature, lyr) => {
                const name = isDistrict
                    ? (feature?.properties?.district || feature?.properties?.DIS_NAME)
                    : (feature?.properties?.upazila || feature?.properties?.UPA_NAME);
                const val = feature?.properties?.[yearKey];
                lyr.bindPopup(`<div style=\"font-family: Arial; font-size: 10px;\">\n                    <b>${isDistrict ? "District" : "Upazila"}:</b> ${name || "Unknown"}<br/>\n                    <b>${yearKey}:</b> ${val ?? "N/A"}\n                </div>`);
            }
        });

        layer.addTo(map);
        layerRef.current = layer;
        if (geojson.features?.length) map.fitBounds(layer.getBounds());
