...
function Alert() {
    ...
    const handleGenerate = async () => {
        setFetching(true);
        console.log("Generating for upazilas:", selectedUpazilas, "and month:", selectedMonth);
        console.log("and month:", selectedMonth);
        try {
            const results = [];
            for (const upaName of selectedUpazilas) {
                const feature = filteredGeoJson.features.find(f =>
                    [f.properties?.UPA_NAME]
                        .map(x => x?.trim().toLowerCase())
                        .includes(upaName.trim().toLowerCase())
                );
                if (!feature || !feature.properties?.UpazilaID) continue;
                const upazila_id = Number(feature.properties.UpazilaID);
                const date = selectedMonth.toISOString().slice(0, 10);

                const res = await axios.post("/predict", {
                    upazila_id, date
                });

                let pv = res.data?.predictions?.pv?.data * 100;
let pf = res.data?.predictions?.pf?.data * 100;
let mixed = res.data?.predictions?.mixed?.data * 100;
        ...
        <div className="flex h-fit flex-col lg:flex-row">
            ...
            <div main className="flex-1 px-4 overflow-y-auto" >
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                    {["PV", "PF", "MIXED"].map(species => (
                        <MapCard
                            key={species}
                            title={`Predicted ${species} case – ${format(selectedMonth, "MMMM yyyy")}`}
                            geojson={filteredGeoJson}
                            forecastResults={forecastResults}                // DO NOT filter!
                            type="forecast"
                            forecastMonth={format(selectedMonth, "yyyy-MM")}
                            threshold={threshold}
                            species={species}                                // <— This drives which data field to use
                        />
                    ))}
...
function GeoJSONLayer({
    ...
    console.log("GeoJSONLayer render", { geojson, forecastResults, forecastMonth, actualData, actualMonth, type, threshold, species });
    ...
    function getPredictionAPI(upaID) {
        if (!forecastResults) return null;
        return forecastResults.find(
            res => Number(res.upazila_id) === Number(upaID)
        );
    }
    ...
    useEffect(() => {
        ...
        const layer = L.geoJSON(geojson, {
            style: feature => {
                ...
                if (type === "forecast") {
                    const pred = getPredictionAPI(upaID);
                    // Match what tab/species is selected:
                    if (pred) {
                        if (species === 'PV') cases = Number(pred.pv ?? 0);
                        else if (species === 'PF') cases = Number(pred.pf ?? 0);
                        else if (species === 'MIXED') cases = Number(pred.mixed ?? 0);
                    }
                } else if (type === "actual") {
                    const actual = getActual(upaID);
                    if (actual) {
               ...
               let fillColor = "#cccccc";  // 0: gray
if (cases === 0) fillColor = "#cccccc";
else if (cases >= 91 && cases <= 100) fillColor = "#bd0026";
else if (cases >= 51 && cases <= 90) fillColor = "#f03b20";
else if (cases >= 31 && cases <= 50) fillColor = "#fd8d3c";
else if (cases >= 11 && cases <= 30) fillColor = "#fecc5c";
else if (cases >= 1 && cases <= 10) fillColor = "#fffbb2";
            ...
            onEachFeature: (feature, lyr) => {
                const upaID = feature?.properties?.UpazilaID;
                const upa = feature?.properties?.UPANAME || feature?.properties?.UPA_NAME;

                let html = `<div style="font-family: Arial, sans-serif; font-size: 11px; color: #333;">
        <table style="width: 100%;">
            <tr>
                ...
                if (type === "forecast") {
                    const pred = getPredictionAPI(upaID);
                    //         console.log("forecastResults", forecastResults);
                    // console.log("Popup for upaID:", upaID, "Prediction lookup result:", pred);
                    if (pred) {
                        html += `
                <tr><td>PV</td><td>${pred.pv ?? "--"}</td></tr>
                <tr><td>PF</td><td>${pred.pf ?? "--"}</td></tr>
                <tr><td>Mixed</td><td>${pred.mixed ?? "--"}</td></tr>
            `;
                    } else {
                        html += `</table><div style="margin-top:2px;color:#777;">No prediction data</div>`;
                    }
                } else if (type === "actual") {
                    const actual = getActual(upaID);
                    if (actual) {
                        html += `
                <tr><td>Month</td><td>${actual.ReportMonth} ${actual.ReportYear}</td></tr>
                <tr><td>${species} Cases</td><td>${actual[species]}</td></tr>
            `;
                    } else {
                        html += `</table><div style="margin-top:2px;color:#777;">No actual data</div>`;
                    }
                }
                lyr.bindPopup(html);
    ...
    }, [geojson, map, forecastResults, forecastMonth, actualData, actualMonth, type, threshold, species]);
...
{species === "PV" && (
  <div style={{ position: "absolute", top: 6, right: 6, zIndex: 3001, background: "#fff", padding: "2px 6px", borderRadius: 6, boxShadow: "0 1px 4px #0001", fontSize: 10, border: "1px solid #eee" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      <span style={{ display: "inline-block", width: 9, height: 9, background: "#cccccc", marginRight: 3, borderRadius: 2, border: "1px solid #bbb" }} /> 0
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      <span style={{ display: "inline-block", width: 9, height: 9, background: "#fffbb2", marginRight: 3, borderRadius: 2, border: "1px solid #ddd" }} /> 1-10
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      <span style={{ display: "inline-block", width: 9, height: 9, background: "#fecc5c", marginRight: 3, borderRadius: 2 }} /> 11-30
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      <span style={{ display: "inline-block", width: 9, height: 9, background: "#fd8d3c", marginRight: 3, borderRadius: 2 }} /> 31-50
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      <span style={{ display: "inline-block", width: 9, height: 9, background: "#f03b20", marginRight: 3, borderRadius: 2 }} /> 51-90
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      <span style={{ display: "inline-block", width: 9, height: 9, background: "#bd0026", marginRight: 3, borderRadius: 2 }} /> 91-100
    </div>
  </div>
)}
