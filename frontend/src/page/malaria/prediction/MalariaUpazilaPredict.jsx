        ...
        fetch("/lmis-data.json")
            ...
            .then(apiData => {
                ...
                if (filtered.length > 0) {
                    toast?.success(`Successfully loaded ${filtered.length} malaria records from local data`);
                }
            })
            .catch(error => {
                console.error("Error loading local LMIS data:", error);
                setActualData([]);
                toast?.error("Failed to load local malaria data. Please check if data file exists.");
            });
    }, [geoJson]);
